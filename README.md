...
`atm_monitor/` — часть проекта, отдельно ничего запускать не нужно: `./run.sh`
поднимает сервер и фоновый сборщик, который раз в час опрашивает
monitoring.btech.uz и пишет данные в SQLite `data/atm_monitor.db` (Postgres больше
не нужен). Для работы задайте `AUTH_USERNAME` / `AUTH_PASSWORD` в `.env`.

- Страница «Менеджер банкоматов»: `/dashboard/atm-monitor.html`
- Живой баланс и кассеты автоматически появляются на карте и в списке ATM
- Опрос вручную: `POST /api/atm-monitor/poll`; один опрос без сервера: `python -m atm_monitor`
...
| `AUTH_USERNAME` / `AUTH_PASSWORD` | — | Логин на monitoring.btech.uz для сборщика atm_monitor. Без них сборщик не стартует (остальное работает) |
| `POLL_INTERVAL_HOURS` | `1` | Как часто сборщик опрашивает BTech |
| `ATM_COLLECTOR` | `1` | `0` — не запускать сборщик вместе с сервером |
| `ATM_MONITOR_DB_PATH` | `data/atm_monitor.db` | SQLite-файл сборщика (реальный баланс/кассеты) |
| `SECRET_KEY` | генерируется временный при каждом запуске | Подписывает cookie сессии — задайте свой, иначе все выходят из системы при каждом перезапуске сервера |

## Авторизация и роли

При первом запуске (пустая таблица `users`) создаётся пользователь `admin` со
случайным паролем — он печатается в лог и сохраняется в `data/bank.db`-соседний
файл `data/admin_credentials.txt` (в `.gitignore`, в git не попадёт). Смените
пароль сразу после первого входа: `/admin` → Пользователи.

Права выдаются не на отдельный HTML-файл, а на функциональный блок (`atm_map`,
`atm_analytics`, `treasury`, `hr_cashiers`) — какие блоки видит роль, задаётся
в самой роли и редактируется в админ-панели (`/admin`, вкладка «Роли»), а не в
коде. Стартовые роли: `admin` (всё), `atm_manager` (карта/инкассация),
`treasury` (касса филиалов), `hr_analyst` (кассиры), `viewer` (без прав по
умолчанию) — админ может их переименовать, изменить состав страниц или
добавить свои.
